...
\subsection{Typing Mathmatical Expressions}
...
This can be written in text as $\widehat{y_i} = \beta_0 + \beta_1 x_{1i}+ \beta_2 x_{2i} + \beta_3 x_{1i} x_{2i} + \epsilon_i.$ using as well.
...
Note, I did a page break here so that the next sections started on a clean page

\pagebreak

\subsection{Runnning R Code}
...
<<size='scriptsize'>>=
8*(9-5)+9 #8(x-5)+ xforx=9
@

Below, we show we produced the code without evalu-
ating it
...
Alternativley, we can produced the output without the 
code.
...
The Result is \Sexpr{round(result.with.error, 5)}. Note that I did not type the
result but I used the \verb|\Sexpr{}| command.

\subsection{Plotting}
We can also plot with R.
<<plot1, eval=FALSE, fig.height=4, size='scriptsize'>>=
#Plota histogramofrandomexponentialdata
hist(rexp(100))
@

\begin{figure}[H]
\begin{center}
<<echo=FALSE, warning=FALSE, message=FALSE, fig.dim=c(5,3,5), fig.height=4>>=
<<plot1>>
@
...
<<size='scriptsize'>>=
 data(VADeaths)
 head(VADeaths) #Takeapeekofthedata
@
